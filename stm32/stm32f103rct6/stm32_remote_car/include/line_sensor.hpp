...
public:
    ...
    /**
     * @brief 设定是否反转传感器顺序（将物理右→左映射为逻辑左→右）
     * @param reverse true=反转（sensor[7]作为逻辑左0），false=不反转
     */
    void setReverseOrder(bool reverse) { reverse_order_ = reverse; }
    ...
    /**
     * @brief 线模式（黑底白线 or 白底黑线）
     */
    enum class LineMode {
        WHITE_ON_BLACK = 0,  // 黑底白线（默认）
        BLACK_ON_WHITE = 1   // 白底黑线
    };

    /**
     * @brief 获取二值化数据（黑白位图）
     * @param binary_data 输出二值化数据数组（8个元素）
     * @param mode 线模式（黑底白线/白底黑线）
     * @param threshold 可选阈值，如果为0则使用校准阈值
     */
    void getBinaryData(bool binary_data[8], LineMode mode = LineMode::WHITE_ON_BLACK,
     ...
     * @param mode 线模式
     * @param threshold 可选阈值，如果为0则使用校准阈值
     * @return 线位置 (-1000 到 1000)，丢线返回NAN
     */
    float getLinePosition(LineMode mode = LineMode::WHITE_ON_BLACK, uint16_t threshold = 0);

    /**
     * @brief 计算线位置并输出传感器数据和二值化数据
     * @param sensor_data 输出传感器原始数据
     * @param binary_data 输出二值化数据
     * @param mode 线模式
     * @param threshold 可选阈值
     * @return 线位置 (-1000 到 1000)，丢线返回NAN
     */
    float getLinePositionWithData(uint16_t sensor_data[8], bool binary_data[8],
                                  LineMode mode = LineMode::WHITE_ON_BLACK, uint16_t threshold = 0);

    /**
     * @brief 检查是否检测到线
     * @param min_sensors 最少需要检测到的传感器数量
     * @param mode 线模式
     * @param threshold 可选阈值
     * @return true=检测到线，false=丢线
     */
    bool isLineDetected(int min_sensors = 1, LineMode mode = LineMode::WHITE_ON_BLACK,
                        uint16_t threshold = 0);
...
private:
    // 校准数据
    uint16_t white_calibration_[8] = {0};  ///< 白色校准值（每个传感器）
    uint16_t black_calibration_[8] = {0};  ///< 黑色校准值（每个传感器）
    uint16_t thresholds_[8] = {0};         ///< 每个传感器的独立阈值（基于校准值计算）
    ...
    // 中值滤波采样次数（默认5次）
    uint8_t median_samples_ = 5;

    // 逻辑顺序反转标志（true 时将物理索引7..0映射为逻辑0..7）
